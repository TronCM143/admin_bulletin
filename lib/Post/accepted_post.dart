...
class AcceptedPosts extends StatefulWidget {
  final String username;

  const AcceptedPosts({super.key, required this.username});

  @override
  _AcceptedPostsState createState() => _AcceptedPostsState();
}

class _AcceptedPostsState extends State<AcceptedPosts> {
  ...
  void _loadAcceptedPosts() async {
    try {
      QuerySnapshot snapshot =
          await FirebaseFirestore.instance.collection('Posts').get();

      List<QueryDocumentSnapshot> tempAcceptedPosts = [];
      ...
      for (var document in snapshot.docs) {
        ...
        if (adminApprovalDoc.exists &&
            adminApprovalDoc['status'] == 'accepted') {
          tempAcceptedPosts.add(document);
        }
      }

      setState(() {
        acceptedDocuments = tempAcceptedPosts;
        isLoading = false;
      });
    } catch (error) {
